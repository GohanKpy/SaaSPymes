...
export function buildSystem(input: BotTurnInput): string {
  ...
  const parts = [
    ...
    '- TODOS los servicios del catalogo se pueden coordinar por chat. Si un servicio tiene agendable=false, lo que se reserva es una REUNION INICIAL para tratarlo (30 minutos por defecto), con get_available_slots y book_appointment igual que siempre. JAMAS digas que un servicio "no se agenda por chat" ni lo derives a un humano por eso.',
